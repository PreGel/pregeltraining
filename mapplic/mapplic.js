	...
	var Mapplic = function() {
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				//add deep linking for slides - replaces href
				
				this.orbitlink = $('<div></div>').attr('data-orbit-link','#').hide().appendTo(this.el);
				this.link = $('<a id="scrolldown">More</a>').addClass('mapplic-tooltip-link').attr('href','#').hide().appendTo(this.orbitlink);

				this.link.on('click touchend', function(e) {
					e.preventDefault();
					 $('html, body').animate({
       					scrollTop: $("#locations").offset().top -200
   					 }, 1000);
					});
			...
			this.set = function(location) {
				if (location) {
					...
					if (location.orbitlink) this.orbitlink.attr('data-orbit-link', location.orbitlink).show();
					//if (location.magarrival) this.magarrival.attr('data-magellan-arrival', location.magarrival).show();
					else this.link.hide();
		...
		var addControls = function() {
			...
			// disable mouse wheel scrolling, but keep panning and click to zoom
